...
const getAllUsers = async (req,res) => {
    ...
    if (user.length>0) return new Response(user, 'bulunan kullanıcılar').success(res)
    else return new Response(null, 'kullanıcı yok').c_404(res)
...
const getUsers = async (req,res) => {
    ...
    if(user === null) return new Response(null, 'kullanıcı bulunamadı').c_404(res)
    else return new Response(user, 'kullanıcı bulundu').success(res)
...
const deleteUsers = async (req,res) => {
    ...
    if(user === null) return new Response(null, 'silinecek kullanıcı bulunamadı').c_404(res)

    if(req.params.id !== req.user._id.toString()) return new Response('null', 'Bu işlem için yetkiniz yok.').c_401(res)

    const deletedUser = await User.findByIdAndDelete(req.user._id)
    return new Response(deletedUser, 'silinen kullanıcı').success(res)
...
module.exports = {
    register, login, uploadTest, forgotPassword,forgotPasswordCheckCode,resetPassword,getUsers, deleteUsers, getAllUsers
}